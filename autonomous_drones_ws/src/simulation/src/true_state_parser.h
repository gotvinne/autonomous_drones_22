...
public:
  ...
  virtual bool ParseMessage(const UnityHeader& header, 
    ...
    pose_msg.header.stamp = ros::Time::now() + ros::Duration(0.5); // ros::Time(header.timestamp + time_offset); ros::Time::now();
    ...
    if (header.name == "Quadrotor/Sensors/IMU"){
      tf->child_frame_id = "drone_1/true_body";
    } 
    else{
       tf->child_frame_id = "drone_2/true_body";
    }
