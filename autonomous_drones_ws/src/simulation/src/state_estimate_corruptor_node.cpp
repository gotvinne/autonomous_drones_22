...
class StateEstimateCorruptorNode {
 public:
	StateEstimateCorruptorNode() {
        ros::NodeHandle pnh("~");
		fla_utils::StringGetParam(pnh, "robot_name", robot_name_);
		fla_utils::SafeGetParam(pnh, "pos_white_sig", pos_white_sigma_);
		fla_utils::SafeGetParam(pnh, "drift_rw_factor", drift_rw_factor_);
		
		pose_sub     = nh.subscribe(topic_tp_, 1, &StateEstimateCorruptorNode::OnPose, this);
		velocity_sub = nh.subscribe(topic_tt_, 1, &StateEstimateCorruptorNode::OnVelocity, this);

		corrupted_pose_pub     = nh.advertise<geometry_msgs::PoseStamped>(topic_pe_, 1);
		corrupted_velocity_pub = nh.advertise<geometry_msgs::TwistStamped>(topic_te_, 1);
		corrupted_state_pub = nh.advertise<nav_msgs::Odometry>(topic_cse_, 1);
 ...
 private:
	...
	void PublishCorruptedState(geometry_msgs::TwistStamped const& corrupt_twist) {
		...
		corrupted_state.child_frame_id = robot_name_ + "/body";
	...
	void PublishBodyTransform(geometry_msgs::PoseStamped const& pose) {
		...
		transformStamped.child_frame_id = robot_name_ + "/body";
	...
	// Topics 
	const std::string topic_tp_ = "true_pose";
	const std::string topic_pe_ = "pose_est";
	const std::string topic_te_ = "twist_est";
	const std::string topic_tt_ = "true_twist";
	const std::string topic_cse_ = "current_state_est";
};

int main(int argc, char* argv[]) {
	ros::init(argc, argv, "StateEstimateCorruptorNode");
	ROS_INFO_ONCE("State machine estimator spawned.");
